...
public class VerifyTools {
  ...
  /** 验证签名 */
  public static boolean verifySign(byte[] message, byte[] signMessage, PublicKey publicKey)
      throws Exception {
    Signature signature;
  ...
  public boolean verifyLicense() throws Exception {
    ...
    // 校验
    boolean verify =
        VerifyTools.verifySign(
            authContent.getBytes(), signature.getBytes(), KeyTools.getPublicKeyFromCer());
